...
// Animated transition (v3.17+)
this.scene.transition({
  target: 'GameOverScene',
  duration: 1000,
  moveAbove: true,
  data: { score: this.score },
  onUpdate: (progress) => { /* fade effect */ }
});

// Switch: sleep current, start target (preserves state)
this.scene.switch('GameScene', 'PauseScene', { pauseData: true });

// Wake a sleeping scene with new data
this.scene.wake('GameScene', { resumed: true });
```

### Scene from plain object (prototyping)

```javascript
// No class needed — useful for quick prototypes
const config = {
  scene: {
