...
**Formula**:
```
imageWidth = (frameWidth x cols) + (spacing x (cols - 1)) + (margin x 2)
imageHeight = (frameHeight x rows) + (spacing x (rows - 1)) + (margin x 2)
```
...
2. **Note total dimensions** (e.g., 448x448 pixels)
3. **Count rows and columns** (e.g., 3x3 grid)
...
**Example**:
- Paper assets: 320x320, 106px frames, no spacing
- Wood Table: 448x448, 144px frames, 8px spacing
...
**Cause**: The art inside each 3x3 cell is centered with lots of transparent padding. Edge frames (3/5 or 1/7) often include a wide region of interior fill, so using the full frame as the slice region paints that fill into the panel as a visible band.
 ...
 *
 * Regular Paper: 320x320, 3x3 grid, 106px frames, no spacing
 * Special Paper: 320x320, 3x3 grid, 106px frames, no spacing
 * Wood Table:    448x448, 3x3 grid, 144px frames, 8px spacing
 *
